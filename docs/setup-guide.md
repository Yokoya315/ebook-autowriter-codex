...
```
Step 1: OpenAI Pro 契約     ← Codex Cloud を使うために必要
...
## 2. OpenAI アカウント作成と Pro 契約

Codex Cloud を使うには **OpenAI Pro プラン**（月額 $200）が必要です。
...
> Codex が表示されない場合は、Pro プランの反映に数分かかることがあります。ページを再読み込みしてください。
...
A: OpenAI Pro プラン（月額 $200）に含まれています。Pro プラン契約中は追加料金なしで Codex Cloud を利用できます。
...
A: はい。Pro プラン契約中は制限なく何冊でも制作できます。テーマを変えて「スタート」と入力するだけです。
